...
#define _usb_isr_invoke_sof_cb(usbp) {                                      \
  if ((usbp) && (usbp)->config && (usbp)->config->sof_cb)                   \
...
#define _usb_isr_invoke_in_cb(usbp, ep) {                                   \
  ...
  if((usbp)->epc[ep]) (usbp)->epc[ep]->in_cb(usbp, ep);                                         \
}
...
#define _usb_isr_invoke_out_cb(usbp, ep) {                                  \
  (usbp)->receiving &= ~(1 << (ep));                                        \
  if((usbp)->epc[ep]) (usbp)->epc[ep]->out_cb(usbp, ep);                                        \
}

#define usb_stall_in(ep)   usb_lld_stall_in(ep)

#define usb_stall_out(ep)  usb_lld_stall_out(ep)
#define usb_clear_in(ep)  usb_lld_clear_in(ep)
#define usb_clear_out(ep)  usb_lld_clear_out(ep)
#define usb_stall_in_sent(ep)   usb_lld_stall_in_sent(ep)
#define usb_stall_out_sent()  usb_lld_stall_out_sent()
#define usb_enable_int()  usb_lld_enable_int()
#define usb_enable_dma_int()  usb_lld_enable_dma_int(void)
#define usb_is_ready(usbp)  ((usbp)->state == USB_ACTIVE)

#define usb_clear_datatoggle(ep, IsInEp) usb_ldd_clear_datatoggle(ep, IsInEp)

#define usb_get_rx_data_len(ep) usb_lld_get_rx_data_len(ep)

#define  usbConnectBus(usbp)      usb_lld_connect_bus(usbp)
#define  usbDisconnectBus(usbp)   usb_lld_disconnect_bus(usbp)
#define  usbSetPowerSessMode(usbp, PwSessMode) usb_lld_set_power_sess_mode(usbp, PwSessMode)

#define usbChkPlugin()            usb_lld_get_plugin()
#define usbChargerEn()            usb_lld_enable_pmu_charger()
#define usbChargerDis()           usb_lld_disable_pmu_charger()
#define usbEnableData()           usb_lld_enData(&USBD1)


//#define usbWriteDmaMode1Patch(usbp, ep) usb_lld_WriteDmaMode1Patch(usbp, ep)
...
/////////////////////////////////////////////////////////////
  ...
  void _usb_ep1_in(USBDriver *usbp, usbep_t ep);
  void _usb_ep1_iout(USBDriver *usbp, usbep_t ep);
