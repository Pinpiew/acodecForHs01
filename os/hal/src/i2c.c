 ...
 * @api
 */
int i2cStart(I2CDriver *i2cp, const I2CConfig *config) {
  int res;
  
  osalDbgCheck((i2cp != NULL) && (config != NULL));
  if((i2cp == NULL) || (config == NULL)) {
    return I2CD_ERROR_NULLPTR;
  }
  ...
  if((i2cp->state != I2C_STOP) 
    ...
    && (i2cp->state != I2C_LOCKED)) {
    return I2CD_ERROR_STATE;
  }

  osalSysLock();
  
  i2cp->config = config;  
  res = i2c_lld_start(i2cp);
  if(res == 0) {
    i2cp->state = I2C_READY;
  }
...
msg_t i2cSlaveReceiveTimeout(I2CDriver *i2cp, const uint8_t *rxbuf, 
			      size_t rxbytes, systime_t timeout) {
  msg_t rdymsg;

  osalDbgCheck((i2cp != NULL) && (rxbytes > 0U) 
...
msg_t i2cSlaveTransmitTimeout(I2CDriver *i2cp, const uint8_t *txbuf, 
  ...
  osalDbgCheck((i2cp != NULL) && (txbytes > 0U) 
	       && (txbuf != NULL) && (timeout != TIME_IMMEDIATE));
 ...
 * @api
 */
msg_t i2cMasterReadMemTimeout(I2CDriver *i2cp, i2caddr_t addr,
            ...
            systime_t timeout)
{
  ...
  i2cp->errors = I2CD_NO_ERROR;
  i2cp->state = I2C_ACTIVE_TX;
  rdymsg = i2c_lld_master_readmem_timeout(i2cp, addr, offset, alen,
                rxbuf, rxbytes, timeout);
  if (rdymsg == MSG_TIMEOUT)
    i2cp->state = I2C_LOCKED;
  else
 ...
 * @api
 */
msg_t i2cMasterWriteMemTimeout(I2CDriver *i2cp, i2caddr_t addr,
			       uint32_t offset, uint8_t alen,
			       const uint8_t *txbuf, size_t txbytes,
			       systime_t timeout)
{
  msg_t rdymsg = MSG_OK;
  uint32_t len, l_len = txbytes;
  ...
  while(l_len) {
    if(l_len > EEPROM_PAGE_SIZE)
      len = EEPROM_PAGE_SIZE;
    else
      len = l_len;    

    rdymsg = i2c_lld_master_writemem_timeout(i2cp, addr, offset, alen,
                              txbuf, len, timeout);    
