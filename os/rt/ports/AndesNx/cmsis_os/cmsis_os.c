...
osStatus osKernelInitialize(void) {
  ...
  chPoolObjectInit(&sempool, sizeof(semaphore_t), chCoreAllocI);
...
osEvent osSignalWait(int32_t signals, uint32_t millisec) {
  osEvent event;
  systime_t timeout = millisec == 0xffffffff ? TIME_INFINITE : MS2ST(millisec);
...
osMessageQId osMessageCreate(const osMessageQDef_t *queue_def,
  ...
  chMBObjectInit(queue_def->mailbox,
                 queue_def->items,
                 (size_t)queue_def->queue_sz / queue_def->item_sz);
 ...
 */
cnt_t osMessageGetUseCount(osMessageQId queue_id) {
  cnt_t c = 0;
  if (port_is_isr_context()) {

    chSysLockFromISR();
    c = chSemGetCounterI(&((mailbox_t *)queue_id)->mb_fullsem);
    chSysUnlockFromISR();
  }
  else
    c = ((mailbox_t *)queue_id)->mb_fullsem.s_cnt;
