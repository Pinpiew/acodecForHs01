...
#define CMSIS_CFG_NUM_SEMAPHORES    20
#endif
...
extern "C" {
#endif
  osStatus osKernelInitialize(void)  __attribute__((used));
  osStatus osKernelStart(void) __attribute__((used));
  osThreadId osThreadCreate(const osThreadDef_t *thread_def, void *argument) __attribute__((used));
  osStatus osThreadTerminate(osThreadId thread_id) __attribute__((used));
  osStatus osThreadSetPriority(osThreadId thread_id, osPriority newprio) __attribute__((used));
  ...
  #endif
  int32_t osSignalSet(osThreadId thread_id, int32_t signals) __attribute__((used));
  int32_t osSignalClear(osThreadId thread_id, int32_t signals) __attribute__((used));
  osEvent osSignalWait(int32_t signals, uint32_t millisec) __attribute__((used));
  osSemaphoreId osSemaphoreCreate(const osSemaphoreDef_t *semaphore_def,
                                  int32_t count) __attribute__((used));
  int32_t osSemaphoreWait(osSemaphoreId semaphore_id, uint32_t millisec) __attribute__((used));
  osStatus osSemaphoreRelease(osSemaphoreId semaphore_id) __attribute__((used));
  osStatus osSemaphoreDelete(osSemaphoreId semaphore_id) __attribute__((used));
  osMutexId osMutexCreate(const osMutexDef_t *mutex_def) __attribute__((used));
  osStatus osMutexWait(osMutexId mutex_id, uint32_t millisec) __attribute__((used));
  osStatus osMutexRelease(osMutexId mutex_id) __attribute__((used));
  osStatus osMutexDelete(osMutexId mutex_id) __attribute__((used));
  osPoolId osPoolCreate(const osPoolDef_t *pool_def) __attribute__((used));
  void *osPoolAlloc(osPoolId pool_id) __attribute__((used));
  void *osPoolCAlloc(osPoolId pool_id) __attribute__((used));
  osStatus osPoolFree(osPoolId pool_id, void *block) __attribute__((used));
  osMessageQId osMessageCreate(const osMessageQDef_t *queue_def,
                               osThreadId thread_id) __attribute__((used));
  osStatus osMessagePut(osMessageQId queue_id,
                        uint32_t info,
                        uint32_t millisec) __attribute__((used));
